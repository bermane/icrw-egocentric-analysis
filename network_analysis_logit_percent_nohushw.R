...
# load data
load(
  '/Users/bermane/Team Braintree Dropbox/Ethan Berman/R Projects/icrw-egocentric-analysis/data/logit_data_table_percent_nohushw.RData'
)
...
write.csv(univar,
          file = '/Users/bermane/Team Braintree Dropbox/ETHAN - ICRW Egocentric data Analysis/Analysis/LOGIT_RESULTS/univar_modern_none_nohushw.csv',
          row.names = F)
...
# output univariate table
write.csv(univar,
          file = '/Users/bermane/Team Braintree Dropbox/ETHAN - ICRW Egocentric data Analysis/Analysis/LOGIT_RESULTS/univar_trad_none_nohushw.csv',
          row.names = F)
...
glm <- glm(
  using_mod_fp ~ parity +
    homo_live_another_village + homo_friends + alt_percent_info_get_fp +
    husband_education + num_alt_not_family + alt_sil*alt_mil,
  family = binomial(link = 'logit'),
  data = dat_mod2
)
summary(glm)
