...
####################################################
### REMOVE HUSBANDS AND HEALTH WORKERS FROM DATA ###
####################################################
...
ea %<>% filter(alt_relationship_cat != 'Health Worker')
...
dat_bihar %<>%
  full_join(
    ea %>%
      group_by(ego_id) %>%
      mutate(alt_sil = sum(str_detect(alt_relationship, 'Sister-in-law'), na.rm = T)) %>%
      select(ego_id, alt_sil) %>%
      distinct
    ,
    by = 'ego_id'
  )
...
# change all sil/mil > 1 to 1
dat$alt_sil[dat$alt_sil > 1] <- 1
dat$alt_mil[dat$alt_mil > 1] <- 1

# factor
dat %<>% mutate(alt_sil = factor(alt_sil),
                alt_mil = factor(alt_mil))

# create sil/mil factor
# dat$alt_sil_mil <- 'None'
# dat$alt_sil_mil[dat$alt_sil == 1 & dat$alt_mil == 0] <- 'SIL'
# dat$alt_sil_mil[dat$alt_sil == 0 & dat$alt_mil == 1] <- 'MIL'
# dat$alt_sil_mil[dat$alt_sil == 1 & dat$alt_mil == 1] <- 'SIL/MIL'
# dat %<>% mutate(alt_sil_mil = factor(alt_sil_mil) %>% relevel(ref = 'None'))
# 
# # remove individual sil mil cats
# dat %<>% select(-c('alt_sil', 'alt_mil'))
...
# as Rdata
save.image('/Users/bermane/Team Braintree Dropbox/Ethan Berman/R Projects/icrw-egocentric-analysis/data/logit_data_table_percent_nohushw.RData')

# as csv
write.csv(dat, '/Users/bermane/Team Braintree Dropbox/Ethan Berman/R Projects/icrw-egocentric-analysis/data/logit_data_table_percent_nohushw.csv',
          row.names = F)
