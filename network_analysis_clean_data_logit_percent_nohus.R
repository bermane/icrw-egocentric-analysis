...
# husband data is excluded to prevent biases
...
# change num_alt_is_hw > 1 to 1 and change col name
dat$num_alt_is_hw[dat$num_alt_is_hw > 1] <- 1
dat %<>% rename(alt_hw = num_alt_is_hw)


# factor
dat %<>% mutate(alt_sil = factor(alt_sil),
                alt_mil = factor(alt_mil),
                alt_hw = factor(alt_hw))
