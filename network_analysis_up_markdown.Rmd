---
title: "Uttar Pradesh Network Analysis"
author: "Ethan Berman"
date: "`r Sys.Date()`"
output:
  html_document:
...
ego <- read_excel(path = "/Users/bermane/Team Braintree Dropbox/Ethan Berman/R Projects/icrw-egocentric-analysis/data/SNA study_EGO_clean_Uttar Pradesh_30June2022.xlsx")
alter <-read_excel(path = "/Users/bermane/Team Braintree Dropbox/Ethan Berman/R Projects/icrw-egocentric-analysis/data/SNA study_ALTER_clean_Uttar Pradesh_09Jul2022.xlsx")
...
ego_pc <- read_excel(path = '/Users/bermane/Team Braintree Dropbox/Ethan Berman/R Projects/icrw-egocentric-analysis/data/ego_pc_complete_up_07122022.xlsx')

# load list of duplicate ids
dup_id <- read_excel(path = '/Users/bermane/Team Braintree Dropbox/Ethan Berman/R Projects/icrw-egocentric-analysis/data/duplicate_ids_up_07112022.xlsx')
...
n <- caste$n[is.na(caste$valid_percent) == F] %>% sum
...
n <- using_fp$n[is.na(using_fp$valid_percent) == F] %>% sum
...
n <- ppl$n[is.na(ppl$valid_percent) == F] %>% sum
...
n <- sex$n[is.na(sex$valid_percent) == F] %>% sum
...
# calculate heterogeneity
res <- tabyl(ea$alt_residence)
n <- res$n[is.na(res$valid_percent) == F] %>% sum
...
# calculate homophily
a <- tabyl(str_detect(ea$alt_learned, "A"))
n <- a$n[is.na(a$valid_percent) == F] %>% sum
...
b <- tabyl(str_detect(ea$alt_learned, "B"))
n <- b$n[is.na(b$valid_percent) == F] %>% sum
...
c <- tabyl(str_detect(ea$alt_learned, "C"))
n <- c$n[is.na(c$valid_percent) == F] %>% sum
...
d <- tabyl(str_detect(ea$alt_learned, "D"))
n <- d$n[is.na(d$valid_percent) == F] %>% sum
...
e <- tabyl(str_detect(ea$alt_learned, "E"))
n <- e$n[is.na(e$valid_percent) == F] %>% sum
...
f <- tabyl(str_detect(ea$alt_learned, "F"))
n <- f$n[is.na(f$valid_percent) == F] %>% sum
...
g <- tabyl(str_detect(ea$alt_learned, "G"))
n <- g$n[is.na(g$valid_percent) == F] %>% sum
...
# calculate heterogeneity
fp <- tabyl(ea$alt_encourage_fp)
n <- fp$n[is.na(fp$valid_percent) == F] %>% sum
